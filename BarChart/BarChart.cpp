...
void CBarChart::DrawTitle(CDC* pDC)
{
	...
	MSYH.CreateFont(19,0,0,0,800,FALSE,FALSE,0,ANSI_CHARSET,
		OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH|FF_SWISS,_T("Î¢ÈíÑÅºÚ"));
	pDC->SetBkMode(TRANSPARENT);
	pDC->SelectObject(&MSYH);

	CRect rcTitle = CRect(m_rctMemPic.left ,
				m_rctMemPic.bottom-CHART_TITLE_HEIGHT, 
				m_rctMemPic.right, m_rctMemPic.bottom-5);
...
void CBarChart::CChartBar::CreateLabelFont()
{
	...
	_tcscpy(m_lf.lfFaceName,_T("Î¢ÈíÑÅºÚ"));
	m_lf.lfHeight=-9;
	m_lf.lfWidth=0;
	m_lf.lfEscapement = 0;
